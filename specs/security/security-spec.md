...
### 6.6 CORS & Origin Protection

> **Implementation Status:** Implemented. Two-layer protection: Fastify CORS with stage-aware origin allowlists + CloudFront `X-Origin-Verify` shared secret header.

**Layer 1 — CORS (browser protection):** Stage-aware origin allowlists enforced at both the API Gateway (preflight) and Fastify (actual requests) levels. No wildcards.

```typescript
// Fastify CORS origins by stage
{
...
**Layer 2 — Origin-verify header (infrastructure protection):** A shared secret stored in Secrets Manager is injected as a `X-Origin-Verify` custom header by CloudFront on every request to the API origin. A Fastify `onRequest` hook validates the header and returns 403 for requests that bypass CloudFront (e.g., direct API Gateway access by non-browser clients).

- Secret: 32-char alphanumeric, generated by `secretsmanager.Secret.generateSecretString`
- CloudFront: `customHeaders: { 'X-Origin-Verify': secret }` on the `/api/*` `HttpOrigin`
- Fastify: `onRequest` hook checks `x-origin-verify` header; skips `/health` and local dev (env var not set)
- WAF deferred per ADR-003; this approach achieves origin protection without WAF cost
...
| CORS origin allowlist + origin-verify header       | Frontend ↔ API integration    | Implemented |
