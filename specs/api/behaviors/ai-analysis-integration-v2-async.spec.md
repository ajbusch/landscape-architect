...
const workerLambda = new lambda.Function(this, 'AnalysisWorker', {
  ...
  layers: [sharpLayer], // Sharp provided via Lambda Layer (see section 6b)
  environment: {
  ...
  bundling: {
    externalModules: ['sharp'], // resolved from layer at runtime
  },
});
...
## 6b. Sharp Lambda Layer

Sharp's ~30MB native binaries make every `cdk synth` and `cdk deploy` painfully slow when bundled into the Worker Lambda. To fix this, Sharp is deployed as a Lambda Layer that only rebuilds when the Sharp version changes.

### Layer structure

```
infra/layers/sharp/
├── build.sh              # builds the layer for Linux x64
└── nodejs/               # generated by build.sh (gitignored)
    └── node_modules/
        └── sharp/
```

### Build script (`infra/layers/sharp/build.sh`)
...
const sharpLayer = new lambda.LayerVersion(this, 'SharpLayer', {
  code: lambda.Code.fromAsset(path.join(__dirname, '../../layers/sharp'), {
    bundling: {
      ...
      command: ['bash', '-c', 'bash build.sh && cp -r nodejs /asset-output/'],
    },
  }),
...
### Worker Lambda bundling

Mark `sharp` as external in the Worker Lambda's esbuild config so it resolves from the layer at runtime:
...
- `sharp` is NOT in `apps/api/package.json` — it comes from the layer
- The `import sharp from 'sharp'` in worker code stays the same — Node resolves it from `/opt/nodejs/node_modules/` at runtime
- The layer only rebuilds when `infra/layers/sharp/build.sh` or its output changes
- Typical code-only deploys bundle in 2-3 seconds instead of 30+
- Add `infra/layers/sharp/nodejs/` to `.gitignore`

---

## 6c. Smoke Test

A standalone script at `apps/api/scripts/test-analysis.ts` exercises the full async analysis flow against a live environment.

### What it does

1. Reads `BASE_URL` from env (default: `https://dev.landscaper.cloud`)
2. Uses a small test JPEG at `apps/api/scripts/fixtures/test-yard.jpg`
3. Calls `POST /api/v1/upload-url` to get a pre-signed URL
4. PUTs the test image to S3 via the pre-signed URL
5. POSTs to `/api/v1/analyses` with `{ photoKey, zipCode: "22903" }`
6. Polls `GET /api/v1/analyses/:id` every 2 seconds, logging each status change
7. On `"complete"`, prints the result summary and exits 0
8. On `"failed"` or timeout after 120s, prints the error and exits 1
...
# Against staging
BASE_URL=https://staging.landscaper.cloud pnpm test:smoke

# Against prod
BASE_URL=https://landscaper.cloud pnpm test:smoke
```

Add `"test:smoke": "tsx apps/api/scripts/test-analysis.ts"` to the root `package.json`.
